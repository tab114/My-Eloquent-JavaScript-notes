...
function every(array, predicate) {
  for (var i = 0; i < array.length; i++) {
    if (!predicate(array[i]))
      return false;
      //if false returned control goes outside the function (since false is returned from the function call)
      //Hence, when any element of array doesn't pass the predicate test, every won't look more elements
  }
  return true;
}
...
function some(array, predicate) {
  for (var i = 0; i < array.length; i++) {
    if (predicate(array[i]))
      return true;
      //if true returned control goes outside the function (since true is returned from the function call)
      //Hence, when any element of array passes the predicate test, some won't look more elements
  }
  return false;
}
